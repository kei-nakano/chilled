class HomeController < ApplicationController
  def top
    ...
    popular_reviews_all = Review.where.not(id: block_ids(@current_user)).where(id: popular_ids).order(['field(id, ?)', popular_ids])
    ...
    @popular_items = Item.where(id: Item.popular_ids).where(id: popular_ids).order(['field(id, ?)', popular_ids]).limit(@item_select)

    # 食べた！の降順に商品を抽出
    eaten_ids = Item.eaten_ids
    @eaten_items = Item.where(id: eaten_ids).order(['field(id, ?)', eaten_ids]).limit(@item_select)

    # 食べてみたい！の降順に商品を抽出
    want_to_eat_ids = Item.want_to_eat_ids
    @want_to_eat_items = Item.where(id: want_to_eat_ids).order(['field(id, ?)', want_to_eat_ids]).limit(@item_select)

    # 1円あたりの内容量(g)が大きい商品を抽出
    reasonable_ids = Item.reasonable_ids
    @reasonable_items = Item.where(id: reasonable_ids).order(['field(id, ?)', reasonable_ids]).limit(@item_select)
